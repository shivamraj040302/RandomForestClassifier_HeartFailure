...
# Make sure 'heart_failure_model.pkl' and 'scaler.pkl' are in the same folder
...
st.image(
    ...
    caption="Heart Health Overview",
    width=250,
    use_container_width=False
)

# Input fields (Aapka format)
age = st.slider("Age", 1, 120, 60)
anaemia = st.selectbox("Anaemia (1=Yes, 0=No)", [0, 1])
creatinine = st.number_input("Creatinine Phosphokinase", 0, 8000, value=500)
diabetes = st.radio("Diabetes", [0, 1], format_func=lambda x: "Yes" if x else "No")
ef = st.number_input("Ejection Fraction", 0, 100, value=35)
hbp = st.radio("High Blood Pressure", [0, 1], format_func=lambda x: "Yes" if x else "No")
platelets = st.number_input("Platelets", 0.0, 1000000.0, value=250000.0)
serum_creatinine = st.number_input("Serum Creatinine", 0.0, 10.0, value=1.0)
serum_sodium = st.number_input("Serum Sodium", 0, 200, value=135)
sex = st.selectbox("Sex (1=Male, 0=Female)", [0, 1])
smoking = st.selectbox("Smoking (1=Yes, 0=No)", [0, 1])
time = st.number_input("Follow-up time (in days)", 0, 300, value=100)

# Prediction button
if st.button("Check Heart Failure Risk"):
    # 1. Inputs ko list mein daalna (Order wahi jo Training mein tha)
    heart_input = [[age, anaemia, creatinine, diabetes, ef, hbp, platelets, 
                    serum_creatinine, serum_sodium, sex, smoking, time]]
    
    # 2. Input ko scale karna (Sabse important step jo missing tha)
    # Isse "Always High Risk" wali problem solve ho jayegi
